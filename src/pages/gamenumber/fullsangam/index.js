...
// let numbers = Array.from(Array(220).keys())
function FullSangam() {
  ...
  const globalSettingApi = () => {
    getGlobalSettings()
      .then(data => {
        if (data.success) {
          setOpenAnk(data.data.gameNumber.fullSangam.openAnk)
          setCloseAnk(data.data.gameNumber.fullSangam.closeAnk)
        } else {
  ...
  return (
    <div>
      <Card sx={{ overflow: 'hidden', marginLeft: '20px', padding: '20px' }}>
        <Typography variant='h6'>Full Sangam Numbers</Typography>
